...
export function RecipeDisplay({ recipe }: RecipeDisplayProps) {
  ...
  const handleCookNow = () => {
    ...
    const instructionsTab = document.querySelector(
      '[value="instructions"]'
    ) as HTMLElement;
    if (instructionsTab) {
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              size="icon"
              className={`rounded-full premium-border ${
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {Object.entries(recipe.nutritionFacts).map(([key, value]) => (
                    <div
                      key={key}
                      className="text-center p-3 bg-muted/50 rounded-md premium-border"
                    >
            ...
            <div>
              ...
              <ul className="space-y-3">
                {recipe.tips.map((tip, index) => (
                  <li
                    key={index}
                    className="flex items-start bg-muted/30 p-3 rounded-lg premium-border"
                  >
                    <span className="mr-2 text-primary">💡</span>
                    <span>{tip}</span>
                  </li>
